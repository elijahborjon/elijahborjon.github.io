...
// Hero postfix rotator
(function () {
  const words = Array.from(document.querySelectorAll("[data-main-header-title-postfix]"));
  if (!words.length) return;

  let idx = 0;
  const activate = (i) => words.forEach((el, k) => el.classList.toggle("is-active", k === i));
...
(function () {
  ...
  const show = (n) => quotes.forEach((q, k) => q.classList.toggle("is-active", k === n));
...
// Custom cursor (MUST be top-level, not inside another handler)
(function () {
  ...
  const hoverTargets = "a, button, .btn, .card, .navlink, .tab, .shot";
